...
Control of logging for pycassa can be performed from the regular python logging
module.  The regular dotted module namespace is used, starting at
'pycassa'.  For class-level logging, the class name is appended.

The "echo" keyword parameter which is available on ``Pool``
objects corresponds to a logger specific to that instance only.
...
is equivalent to::
    ...
    import logging
    logger = logging.getLogger('pycassa.pool.Pool.%s' % hex(id(engine)))
...
rootlogger = logging.getLogger('pycassa')
